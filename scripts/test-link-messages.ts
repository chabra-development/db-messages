...
async function syncSingleTicket(
  ...
  while (hasMore) {
    if (skip >= startSkip + MAX_SKIP) {
      console.log(
        `        [skip=${skip}] limite de 10.000 atingido — ticket adiado`,
      );
...
async function main() {
  ...
  for (const ticket of tickets) {
    ...
    console.log(
      `    Mensagens retornadas pelo Blip (amostra): ${sample.length}`,
    );

    if (sample.length > 0) {
      const msg = sample[0];
      const resolvedId =
        (msg.metadata?.["#messageId"] as string | undefined) ?? msg.id;
      console.log(`    message.id:               ${msg.id}`);
      console.log(
        `    metadata["#messageId"]:   ${msg.metadata?.["#messageId"] ?? "(ausente)"}`,
      );
    ...
    const { synced, notFound, deferred, nextSkip } = await syncSingleTicket(
      ticket.id,
      ticket.blipId,
    );
    ...
    if (deferred) {
      console.log(
        `    ⚠ ADIADO — synced: ${synced} | notFound: ${notFound} | continuará em skip=${nextSkip}\n`,
      );
      deferredTickets.push({
  ...
  if (deferredTickets.length > 0) {
    ...
    for (const t of deferredTickets) {
      console.log(
        `  #${t.sequentialId} — blipId: ${t.blipId} — continuará em skip=${t.startSkip}`,
      );
    }
    console.log();
    ...
    for (const t of deferredTickets) {
      console.log(
        `--- Reprocessando #${t.sequentialId} a partir de skip=${t.startSkip} ---`,
      );
      const {
        synced,
        notFound,
        ...
        nextSkip,
      } = await syncSingleTicket(t.ticketId, t.blipId, t.startSkip);
      if (stillDeferred) {
        console.log(
          `    ⚠ AINDA ADIADO — synced: ${synced} | notFound: ${notFound} | próximo skip=${nextSkip}\n`,
        );
      } else {
        console.log(
          `    ✓ CONCLUÍDO — synced: ${synced} | notFound: ${notFound}\n`,
        );
      }
    }
  }
