...
async function importSingleTicket(
  ...
  while (true) {
    ...
    console.log(
      `    [skip=${skip}] msgs=${messages.length} | created=${toCreate.length} | linked=${toLink.length}`,
    );
...
async function main() {
  ...
  for (const ticket of tickets) {
    ...
    if (!contactId) {
      console.log(
        "    ✗ Contato não encontrado (sem originator ou não importado) — pulando\n",
      );
      continue;
    }
    ...
    if (deferred) {
      console.log(
        `    ⚠ ADIADO — created: ${created} | linked: ${linked} | notFound: ${notFound}`,
      );
    } else {
      console.log(
        `    ✓ created: ${created} | linked: ${linked} | notFound: ${notFound}`,
      );
    }
