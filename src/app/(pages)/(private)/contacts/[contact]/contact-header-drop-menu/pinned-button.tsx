...
export const PinnedButton = ({ contactId, preference }: PinnedButtonProps) => {
  const { mutate } = useMutation({
    mutationKey: ["toggle-pinned-contact"],
    mutationFn: togglePinnedContact,
    onSuccess: ({ pinned }) => {
      toast({
        title: `Contato ${pinned ? "adicionado aos" : "retirado dos"} fixados`,
      });
  ...
  return (
    <DropdownMenuItem onClick={handleTogglePinnedContact}>
      ...
      {myPreferenceExist ? "Retirar dos fixados" : "Fixar contato"}
    </DropdownMenuItem>
  );
};
