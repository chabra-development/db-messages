...
export const SupabaseStatsQuery = () => {
  ...
  const sentCount =
    (
      messagesByDirection.find((m) => m.direction === "SENT")?._count as
        | { _all: number }
        | undefined
    )?._all ?? 0;
  const receivedCount =
    (
      messagesByDirection.find((m) => m.direction === "RECEIVED")?._count as
        | { _all: number }
        | undefined
    )?._all ?? 0;
    ...
    <div className="space-y-6 p-1">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-5 gap-3">
        {statCards.map(({ label, value, icon: Icon, href }) => (
          <Card
            key={label}
            className={`gap-2 ${href ? "hover:border-primary transition-colors" : ""}`}
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              {totalMessages.toLocaleString("pt-BR")} no total
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-3">
            <div className="space-y-1">
              <div className="flex items-center justify-between text-sm">
                ...
                <span className="font-medium">
                  {sentCount.toLocaleString("pt-BR")}
                </span>
              </div>
            ...
            <div className="space-y-1">
              <div className="flex items-center justify-between text-sm">
                ...
                <span className="font-medium">
                  {receivedCount.toLocaleString("pt-BR")}
                </span>
              </div>
              <div className="h-2 rounded-full bg-muted overflow-hidden">
                <div
                  ...
                  style={{
                    width: totalMessages
                      ? `${(receivedCount / totalMessages) * 100}%`
                      : "0%",
                  }}
                />
              </div>
          ...
          <CardHeader>
            <div className="flex items-center justify-between">
              <div>
                ...
                <CardDescription>
                  {counts.tickets.toLocaleString("pt-BR")} no total
                </CardDescription>
              </div>
              <Link
                href={"/tickets" as Route}
                className="text-muted-foreground hover:text-foreground transition-colors text-sm"
                title="Ir para Tickets"
              >
                ↗
              </Link>
            </div>
                ...
                <div key={status} className="space-y-1">
                  ...
                  <div className="h-1.5 rounded-full bg-muted overflow-hidden">
                    <div
                      ...
                      style={{
                        width: counts.tickets
                          ? `${(count / counts.tickets) * 100}%`
                          : "0%",
                      }}
                    />
                  </div>
