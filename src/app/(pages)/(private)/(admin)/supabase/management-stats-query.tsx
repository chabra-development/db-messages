  ...
  if (!data || !data.configured) {
    return (
      <CardDescription className="italic">
        ...
        no <code className="text-xs bg-muted px-1 py-0.5 rounded">.env</code>{" "}
        para visualizar estas informações. Gere o token em{" "}
        <strong>supabase.com/dashboard/account/tokens</strong>.
      </CardDescription>
    );
  }
      ...
      {project && (
        <div className="flex flex-wrap gap-4 text-sm">
          <span className="text-muted-foreground">
            Nome: <strong className="text-foreground">{project.name}</strong>
          </span>
          <span className="text-muted-foreground">
            Região:{" "}
            <strong className="text-foreground">{project.region}</strong>
          </span>
          <span className="text-muted-foreground">
            Status:{" "}
            <Badge
              variant={
                project.status === "ACTIVE_HEALTHY" ? "default" : "destructive"
              }
              className="ml-1"
            >
              {project.status}
      ...
      {health && health.length > 0 && (
        <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-2">
          {health.map((item) => (
            ...
            >
              <span className="text-muted-foreground">
                {SERVICE_LABEL[item.name] ?? item.name}
              </span>
              <div className="flex items-center gap-1.5">
                ...
                <Badge
                  variant={STATUS_BADGE[item.status] ?? "outline"}
                  className="text-xs"
                >
                  {item.status}
                </Badge>
