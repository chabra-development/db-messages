...
export const AttendantCard = ({ index, attendant }: AttendantCardProps) => {
  const { id, name, email, role, teams } = attendant;
      ...
      <Card className="h-full justify-between">
        <CardHeader>
          ...
          <div className="flex items-center w-full gap-2.5">
            ...
            <Badge className={cn("text-zinc-50", className)}>{title}</Badge>
          </div>
          <CardAction>
            <ChangeRoleUserDialog user={attendant} />
          </CardAction>
        </CardHeader>
        <ScrollArea className={cn(teams.length > 3 ? "h-36" : "h-20")}>
