      ...
      for (const ticket of tickets) {
        try {
          const {
            ...
            deferred: isDeferred,
            nextSkip,
          } = await syncSingleTicketMessages(ticket.id, ticket.blipId);
          totalSynced += synced;
...
async function syncSingleTicketMessages(
  ticketId: string,
  blipId: string,
  startSkip = 0,
) {
...
async function processLinkDeferredMessages(
  jobId: string,
    ...
    for (const ticket of tickets) {
      try {
        const {
          ...
          deferred: isDeferred,
          nextSkip,
        } = await syncSingleTicketMessages(
          ticket.ticketId,
          ticket.blipId,
          ticket.startSkip,
        );
...
export async function getLinkStatistics() {
  const [linked, unlinked, total, ticketsWithMessages, ticketsTotal] =
    await Promise.all([
