...
const MIME_PNG_MAP: Record<string, string> = {
    ...
    "application/vnd.openxmlformats-officedocument.wordprocessingml.document": "/files-types/docx.svg",

    // Excel / Planilhas
    "application/vnd.ms-excel": "/files-types/xls.svg",
    "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet": "/files-types/xls.svg",
...
function getLucideFallback(mimeType: string): LucideFallbackMeta {
    ...
    if (category === "application") return { icon: FileSpreadsheet, color: "#6B7280" }
...
export function FileTypeIcon({ mimeType, size = 24, color, className, ...props }: FileTypeIconProps) {
    ...
    return <Icon size={size} color={color ?? defaultColor} className={className} {...props} />
}
