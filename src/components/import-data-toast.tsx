...
export function ImportProgressToast({
  ...
  useEffect(() => {
    if (isComplete && data) {
      ...
      toast.custom(
        (t) => (
          <SuccessToastContent data={data} onDismiss={() => toast.dismiss(t)} />
        ),
        {
  ...
  useEffect(() => {
    if (hasFailed && data) {
      ...
      toast.custom(
        (t) => (
          <ErrorToastContent data={data} onDismiss={() => toast.dismiss(t)} />
        ),
        {
...
function SuccessToastContent({ data, onDismiss }: SuccessToastContentProps) {
  ...
  const deferred = Array.isArray(data.metadata?.deferred)
    ? data.metadata.deferred
    : null;
      ...
      <div className="size-full space-y-4">
        ...
        {deferredCount > 0 && (
          <div className="p-3 rounded-md bg-yellow-500/10 border border-yellow-500/20 text-xs text-yellow-700 dark:text-yellow-400">
            {deferredCount} ticket(s) com mais de 10.000 mensagens foram
            adiados. Use "Reprocessar adiados" para continuar a vinculação.
          </div>
        )}
