...
import {
  ...
  CreateTagsProps,
  createTagsSchema,
} from "@/schemas/create-tags-schema";
...
export const FormCreateContactTags = ({
  ...
  const [currentContactTag, setCurrentContactTag] = useQueryState(
    "name",
  ...
  const { fields, append, remove } = useFieldArray<CreateTagsProps>({
    name: "tags",
    control,
  });
            ...
            <CardContent className="px-4 py-0 flex items-center flex-wrap gap-2">
              {allTags.length === 0 ? (
                <CardDescription className="text-sm">
                  Nenhuma tag disponível. Crie tags para organizar melhor seus
                  contatos.
                </CardDescription>
              ) : (
              ...
              fields.map(({ id, name }, i) => (
                ...
                >
                  ...
                  <CardFooter className="px-1.5">
                    <Button size={"icon-xs"} onClick={() => remove(i)}>
                      <X className="size-4" />
                    </Button>
                  </CardFooter>
                </Card>
